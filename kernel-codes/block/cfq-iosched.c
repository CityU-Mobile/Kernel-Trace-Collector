...
#if ((defined CONFIG_HUBERY_TRACE_BIO_READ) || (defined CONFIG_HUBERY_TRACE_BIO_WRITE))
	{
			if (rq->bio) {
				...
				char * fname;
				char * ss;
				if (bio_op(rq->bio) == READ) {
#ifdef CONFIG_HUBERY_TRACE_BIO_READ
								...
								if (inode) {
									if (inode->i_dentry.first) {
										...
										if (ss) {
											...
											do_gettimeofday(&tv);
											printk(
												KERN_INFO "[hubery] [%d]%ld,%s,bio-r,%lu,%u,%lu,%d,%lu\n",
												i,
												(tv.tv_sec * 1000 + tv.tv_usec / 1000),
												ss, inode->i_ino,
												rq->bio->bi_iter.bi_size,
												rq->bio->bi_iter.bi_sector,
												rq->bio->bi_vcnt,page->index);
											}
										}
										kfree(fname);
									}
...
#ifdef CONFIG_HUBERY_TRACE_BIO_WRITE
					if (rq->bio->bi_io_vec && rq->bio->bi_io_vec->bv_page
							&& rq->bio->bi_vcnt > 0) {
						bio_for_each_segment_all(bvec, rq->bio, i)
						{
							struct page *page = bvec->bv_page;
							/* if(rq->bio->bi_private) {
								do_gettimeofday(&tv);
								printk(
									KERN_INFO "[hubery] [%d]%ld,%s,bio-w,%lu,%u,%lu,%d\n",
									i,
									(tv.tv_sec * 1000 + tv.tv_usec / 1000),
									"compr write endio", 1,
									rq->bio->bi_iter.bi_size,
								...
								if (inode) {
									if (inode->i_dentry.first) {
										...
										if (ss) {
											...
											do_gettimeofday(&tv);
											printk(
												KERN_INFO "[hubery] [%d]%ld,%s,bio-w,%lu,%u,%lu,%d,%lu\n",
												i,
												(tv.tv_sec * 1000 + tv.tv_usec / 1000),
												ss, inode->i_ino,
												rq->bio->bi_iter.bi_size,
